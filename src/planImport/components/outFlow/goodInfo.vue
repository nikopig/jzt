    ...
    <div class="common-container">
      ...
      <div class="foot-bar">
        <div class="btns-content">
          <el-button type="button" class="el-button el-button--primary" @click="putinCar">
            <span>加入订单车</span>
          </el-button>
        </div>
      </div>
...
<script>
    ...
    import {getIndexOfCollection} from '@/common/js/utils.js'
    ...
    export default {
        ...
        computed: {
          ...
          selectData () {
            return this.goods.map((item) => {
              ...
              temp.OutbAddress_Id = this.Ldc_Address_Id
              temp.Delivery_Address_Id = this.Address_Id
        ...
        methods: {
          putinCar () {
            ...
            this.StoreCartAdd()
              .then((resp) => {
                ...
                if (resp.Flag) {
                  this.$router.push('/outFlow/myOrder')
                } else {
          ...
          onChange (val) {
            let pos = getIndexOfCollection('Goods_Id', val.Goods_Id, this.goods)
            if (val.checked) {
              if (pos === -1) this.goods.push(val)
            } else {
              if (pos !== -1) this.goods.splice(pos, 1)
            }
          },
          // 加入订单车
          StoreCartAdd () {
            ...
            return Api.post('OutCartAdd', params)
          },
          // 检索商品
          GetStorageGoods () {
            var params = {
            ...
            Api.get('GetOutboundGoods', params)
              .then((resp) => {
                this.loadingWating.close()
                if (resp.Flag) {
