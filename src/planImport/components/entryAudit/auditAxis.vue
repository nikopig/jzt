    ...
    <div>
      <common-time-table @point="onPoint" @changeDate="onChangeDate" :data="realData" :start="startDate" :section="section">
        ...
        <template slot="body" slot-scope="props">
          <label @click="goDetail(props.row.id, props.detail.date, 'A1')">待货：<span>{{props.detail.outSend}}</span></label>
          <label @click="goDetail(props.row.id, props.detail.date, 'A2')">提货：<span>{{props.detail.outStay}}</span></label>
        </template>
      </common-time-table>
    </div>
</template>
          ...
          realData () {
            ...
            this.origData.forEach((item) => {
              ...
              if (pos === -1) {
                ...
                temp.dataList = []
                temp.dateList = []
                temp.dateList.push(item.Takegds_Time)
                temp.dataList.push(this.createObj(item))
                rel.push(temp)
              } else {
                let dataList = rel[pos].dataList
                let dateList = rel[pos].dateList
                let pos1 = dateList.indexOf(item.Takegds_Time)
                if (pos1 === -1) {
                  dateList.push(item.Takegds_Time)
                  dataList.push(this.createObj(item))
                } else {
                  // 1 待货入库 2 提货入库
                  if (item.Arrival_Mode === 'A1') dataList[pos1].outSend = item.Cnt
                  if (item.Arrival_Mode === 'A2') dataList[pos1].outStay = item.Cnt
                }
              }
            })
            console.log(rel)
...
//            }
          ...
          // 获取最早审核列表时间
          getStartTime () {
            return Api.get('JlpGetStoreABillFirst', {})
          },
          // 获取数据
          getData () {
            let params = {
            ...
            Api.get('JlpGetStoreABillList', params)
              .then((resp) => {
                if (resp.Flag) {
          ...
          createObj (item) {
            ...
            tempObj.date = item.Takegds_Time
            // 1 待货入库 2 提货入库
            if (item.Arrival_Mode === 'A1') tempObj.outSend = item.Cnt
            if (item.Arrival_Mode === 'A2') tempObj.outStay = item.Cnt
            return tempObj
          },
          ...
          onPoint (date) {
            this.$emit('viewDetail', {
              Con_Id: '%',
              Arrival_Mode: '%',
              Takegds_Time: date === 'hisdata' ? this.startDate : date,
              // 是否为历史数据
...
//                    if (!resp.MsgInfo[0].Takegds_Time || resp.MsgInfo[0].Takegds_Time === '0001-01-01') {
//                      return false
//                    }
//                    this.startDate = resp.MsgInfo[0].Takegds_Time
//                  }
//                }
//                this.getData()
//              })
